...
- `2026-09-24` self-found repo-audit gap (defunct MockHero entry) — outcome: pr-opened (fork PR #7) — live HTTP sweep found `https://mockhero.dev` returns 404 on every path and the product is discontinued (domain parked on a placeholder IP; mockhero.io / www / api mockhero.dev no longer resolve). No working replacement or successor url. Deduped against upstream PR history: no open/closed/merged PR addresses the dead MockHero link (PR #109 added it, PR #159 updated its description). Removal sanctioned by config link_check.replacement_rule (defunct + no successor -> delete, and say so). Fork PR #7 base=olitreadwell/awesome-testing master, non-draft, mergeable clean; marked as 'Mined gaps: mockhero.dev dead link — dropped (trivial loop)' now attempted as a main-loop docs-grounded PR.
